...
async function okfyHomeForSafety() {
...
async function buildActivationPacket(options) {
  ...
  const files = PACKET_FILES.map((file) => ({ ...file, path: path2.join(outDir, file.fileName) }));
...
async function ensureActivationOutDir(outDir, options) {
  ...
  try {
    const entries = await fs2.readdir(outDir);
    if (entries.length > 0) {
      if (!options.force)
        throw new Error(
...
function assertActivationOutDirDoesNotTargetProtectedPaths(outDir, protectedInputPaths) {
  const resolvedOut = path2.resolve(outDir);
  let nestedConflict;
  for (const inputPath of protectedInputPaths) {
    const protectedPath = path2.resolve(inputPath);
...
function uniqueResolvedPaths(paths) {
  return Array.from(new Set(paths.map((filePath) => path2.resolve(filePath))));
}
async function writeFileAtomically(filePath, contents) {
  ...
  try {
    ...
    await fs2.rename(tempPath, resolved);
  } catch (error) {
...
function tempBundleDir(sourceDir) {
  return path6.join(sourceDir, `bundle.tmp-${process.pid}-${randomUUID()}`);
}
function lockfilePath(sourceDir) {
  return path6.join(sourceDir, ".refresh.lock");
}
...
async function replaceActiveBundle(tempDir, bundleDir) {
  ...
  try {
    await fs5.mkdir(path6.dirname(bundleDir), { recursive: true });
