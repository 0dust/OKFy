...
- uses non-empty string `title`, `description`, and `type` properties, and string or string-array `aliases` when resolving note identity
- accepts string or string-array `tags` and merges them with inline and inferred tags, while ignoring tag-like text in code, HTML, and MDX expressions
- warns with `invalid_frontmatter_property` and uses fallbacks for incompatible recognized values; malformed YAML is a document-level import error
...
Missing and ambiguous targets are never guessed. Their original Obsidian references remain readable and surface `unresolved_wikilink` or `ambiguous_wikilink` warnings. Missing headings and block IDs surface `missing_wikilink_fragment`. These semantic warnings do not make an otherwise valid bundle invalid; inspect them with `okfy validate`, Inspector, or `bundle_summary`. Recognized-property warnings are reported during import.
