...
- Source YAML `title`, `description`, and `type` override inferred values when they are non-empty strings. String or string-array `aliases` contribute to note identity. Source and inline `tags` augment inferred tags and are deduplicated case-insensitively. Other safe YAML properties are retained deterministically; okfy owns `resource` and `timestamp`.
