...
describe("setup client artifacts", () => {
  it("renders npx serve commands without OKFY_HOME for the default source store", () => {
    ...
    const claude = renderClientArtifacts({ client: "claude-code", sourceName: "stripe", okfyHome })[0];
    ...
    expect(claude.body).toBe("claude mcp add --transport stdio stripe-okf -- npx -y okfy-ai serve stripe --mcp --auto-refresh");
  });

  it("renders option-like legacy source names without broken CLI parsing", () => {
    const okfyHome = defaultOkfyHome();
    const command = serveCommand("-legacy", okfyHome);
    const claude = renderClientArtifacts({ client: "claude-code", sourceName: "-legacy", okfyHome })[0];
    const codex = renderClientArtifacts({ client: "codex", sourceName: "-legacy", okfyHome });

    expect(command.args).toEqual(["-y", "okfy-ai", "serve", "--mcp", "--auto-refresh", "--", "-legacy"]);
  ...
  it("renders OKFY_HOME overrides for non-default source stores", () => {
    ...
    const codexArtifacts = renderClientArtifacts({ client: "codex", sourceName: "stripe", okfyHome });
    const codex = codexArtifacts[0];
    const codexCli = codexArtifacts.find((artifact) => artifact.label === "Codex CLI");
    const claude = renderClientArtifacts({ client: "claude-code", sourceName: "stripe", okfyHome })[0];
  ...
  it("renders every supported client family and normalizes aliases", () => {
    ...
    const okfyHome = defaultOkfyHome();
    expect(renderClientArtifacts({ client: "claude-code", sourceName: "stripe", okfyHome })[0].format).toBe("shell");
    expect(renderClientArtifacts({ client: "mcp-json", sourceName: "stripe", okfyHome })[0].format).toBe("json");
    expect(renderClientArtifacts({ client: "codex", sourceName: "stripe", okfyHome }).map((artifact) => artifact.format)).toEqual([
      "toml",
      "shell"
    ]);
    expect(renderClientArtifacts({ client: "generic", sourceName: "stripe", okfyHome })[0].format).toBe("json");
  });
  ...
  it("prints first prompt guidance and structured report severity", () => {
    ...
    const report = createSetupReport({
      ...
      checks: [
        ...
        setupCheck("freshness", "Freshness", "warn", "Source is stale.", "Run npx -y okfy-ai update stripe."),
...
describe("MCP probe message evaluation", () => {
  it("passes only when expected read-only tools are visible", () => {
    ...
    const failing = evaluateMcpProbeMessages([{ id: 2, result: { tools: [{ name: "bundle_summary" }] } }]);
